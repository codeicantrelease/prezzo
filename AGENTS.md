...
- Treat runtime chrome as deck opt-in. Timers, the Quake terminal, and command-started overlays live under `src/runtime/` and should be enabled from `deck.config.ts` only when useful.
